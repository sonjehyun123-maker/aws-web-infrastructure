...
브라우저 검증과 함께 CLI에서도 HTTP 상태 코드와 응답 본문을 확인할 수 있습니다.
...
예상 결과는 `HTTP 200 OK`이며 `/health` 응답 본문은 `OK`입니다.

SSH 접속도 다음 명령으로 확인할 수 있습니다.

```bash
ssh -i mission-key.pem ubuntu@3.34.42.120
```

접속 후 다음 명령으로 서버 상태를 확인합니다.
...
SSH 허용 IP가 변경되는 경우 현재 공인 IP를 확인한 뒤 기존 `/32` 규칙을 새 IP로 변경합니다.

```powershell
$MY_IP = (Invoke-RestMethod -Uri "https://checkip.amazonaws.com").Trim() + "/32"
```

이전 IP를 그대로 누적해서 허용하기보다 사용하지 않는 규칙을 제거하고 현재 관리자 IP만 유지합니다.

### HTTP 공개 및 Outbound 설정

HTTP 80은 외부 웹 서비스 제공을 위해 `0.0.0.0/0`으로 공개되어 있습니다. 이 설정은 인터넷상의 포트 스캔이나 웹 서버 취약점 공격에 노출될 가능성이 있으므로 실제 운영 환경에서는 HTTPS, WAF 등의 추가 보안 설정을 고려할 수 있습니다.

현재 Outbound 전체 허용은 패키지 업데이트와 외부 통신을 위한 실습 설정입니다. 운영 환경에서는 필요한 목적지와 포트만 허용하는 방식으로 제한할 수 있습니다.

### 리소스 이름 및 태그 기준

추가 리소스를 생성할 경우 기존 `Mission-*` 형식을 유지합니다.
...
권한을 추가할 때는 필요한 작업부터 최소 범위로 허용하고, 권한 확대 후에는 CloudTrail 등의 감사 기록을 확인하여 불필요한 권한이 남지 않았는지 점검합니다.

### 외부 접속 장애 점검 순서

외부 HTTP 접속이 실패할 경우 다음 순서로 점검합니다.
...
현재 구조는 단일 EC2 기반이므로 CPU 사용률, 메모리 사용량, 네트워크 트래픽, 동시 접속량, Nginx 응답 지연 및 HTTP 5xx 발생량 등을 확인하여 병목 여부를 판단할 수 있습니다.

단일 EC2가 병목이 되어 여러 서버로 확장할 필요가 생기면 ALB와 Target Group을 추가할 수 있습니다.
...
비용 확인 시 EC2뿐 아니라 EBS, Elastic IP, Load Balancer, NAT Gateway 등의 잔여 리소스도 함께 확인합니다.
