...
SSH 접근 IP가 변경되면 현재 공인 IP를 확인하여 기존 `/32` 규칙을 새 IP로 변경합니다.

```powershell
$MY_IP = (Invoke-RestMethod -Uri "https://checkip.amazonaws.com").Trim() + "/32"
```

이전 IP를 계속 추가하기보다 사용하지 않는 규칙을 제거하여 SSH 접근 범위를 최소화합니다.

### Security Group 공개 범위

HTTP 80은 외부 웹 서비스 제공을 위해 `0.0.0.0/0`으로 공개되어 있습니다. 인터넷 전체 공개는 포트 스캔이나 웹 서버 취약점 공격에 노출될 수 있으므로 실제 운영 환경에서는 HTTPS, WAF 등의 추가 보안 설정을 고려할 수 있습니다.

Outbound 전체 허용은 현재 실습에서 패키지 업데이트와 외부 통신을 위해 사용한 설정입니다. 운영 환경에서는 필요한 목적지와 포트만 허용하는 방식으로 축소할 수 있습니다.

### IAM 권한 관리

Security Group은 네트워크 접근을 제어하고 IAM은 AWS API 작업 권한을 제어합니다.
...
외부에서는 다음과 같이 원본 HTTP 응답을 확인합니다.
...
병목 여부는 CPU 사용률, 메모리 사용량, 네트워크 트래픽, 동시 접속량, Nginx 응답 지연 및 HTTP 5xx 발생량 등을 확인하여 판단할 수 있습니다.

여러 EC2로 확장할 필요가 생기면 ALB와 Target Group을 추가할 수 있습니다.
...
실습 종료 후에는 EC2, EBS, Elastic IP, Load Balancer, NAT Gateway 등의 잔여 리소스를 확인하고 Billing & Cost Management에서 최종 비용을 확인합니다.
